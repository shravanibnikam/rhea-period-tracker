...
import type { PhaseData, LegacyCycleEntry, DailyLog, Cycle } from "@/types";
import { PHASES } from "@/lib/phases";
import { parseDate, fmt } from "@/lib/utils";
import { analyzeSymptomPatterns, getVariabilityLabel, type SymptomPattern } from "@/lib/cycle";
...
export function HistoryTab({
  ...
  const completedCycles = cycleData.filter((c) => c.length !== null);

  const chartData = completedCycles.map((c) => ({
    name: parseDate(c.start).toLocaleDateString("en-US", {
      month: "short",
      year: "2-digit",
    }),
    days: c.length,
    flow: c.flow,
    excluded: excludedStarts.has(c.start),
  }));
    ...
    <div className="space-y-5">
      ...
      <div className="bg-card rounded-2xl border border-border p-6">
        ...
        <div className="space-y-0">
          {[...cycleData].reverse().map((c, i) => {
            const isCurrent = i === 0;
            const start = parseDate(c.start);
            const isExcluded = excludedStarts.has(c.start);
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{ backgroundColor: flowBg[c.flow] }}
                  />
                  <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {onToggleExcluded && c.length && (
                    <button
                      onClick={() => onToggleExcluded(c.start)}
        ...
        <div className="bg-card rounded-2xl border border-border p-6">
          ...
          <div className="space-y-3">
            {symptomPatterns.slice(0, 8).map((sp) => {
              const peakColor = PHASES[sp.peakPhase].color;
