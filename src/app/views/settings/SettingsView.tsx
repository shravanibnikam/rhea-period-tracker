...
import { downloadJSON } from "@/data/exporter";
import { isRheaError } from "@/kernel";
import { useContainer } from "@/app/di";
import { parseImportFile, sourceLabel, type ImportResult } from "@/data/importer";
import { PairingSection } from "./PairingSection";
import { SharingControls } from "./SharingControls";
import { SyncTransportSection } from "./SyncTransportSection";
...
export function SettingsView({
  ...
  const handleExport = async () => {
    const data = await container.exportBackup();
    downloadJSON(data);
  ...
  const handleRheaBackupImport = () => {
    ...
    input.onchange = async (e) => {
      ...
      if (!file) return;
      try {
        // Accepts version 1 or 2; newer versions are rejected with clear copy.
        const { imported, skipped } = await container.importBackup(await file.text());
        onDataChanged();
        showStatus(
          skipped > 0
            ? `Imported ${imported} days (${skipped} already up to date)`
            : `Imported ${imported} days from Rhea backup`
        );
      } catch (err) {
        showStatus(isRheaError(err) ? err.userMessage : "Failed to read backup file");
      }
    };
    input.click();
  };
  ...
  const confirmImport = async () => {
    ...
    // Merge into existing logs (never blind-overwrite a rich local entry).
    const { imported } = await container.importParsedLogs(importPreview.logs);
    onDataChanged();
    showStatus(
      `Imported ${imported} days from ${sourceLabel(importPreview.source)}`
    );
