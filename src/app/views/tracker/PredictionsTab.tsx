import type { PredictedCycle } from "@/domain/types";
import {
  PHASES,
  PHASE_ORDER,
  getPhaseLengths,
...
export function PredictionsTab({
  ...
  // Single phase oracle (M1.3): widths + per-phase date ranges all derive from
  // the same boundaries the rest of the app uses.
  const anchors = anchorsFrom(avgLength, avgPeriodLength);
  const phaseLengths = getPhaseLengths(anchors);
  const boundaries = getPhaseBoundaries(anchors);
          ...
          {predictions.map((pred, i) => (
            <div key={i} className="border border-border rounded-2xl p-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {PHASE_ORDER.map((p) => {
                  const startOffset = boundaries[p].startDay - 1;
                  const endOffset = boundaries[p].endDay - 1;
          ...
          {PHASE_ORDER.map((p) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  {getPhaseRangeLabel(p, anchors)} &middot; {PHASES[p].tagline}
                </p>
              </div>
